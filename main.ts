 ...
 */
async function cloneRepository(repoUrl: string, projectPath: string): Promise<string> {
  if (existsSync(projectPath)) {
    ...
    if (gitDirExists) {
      console.log(`Repository already cloned. Pulling latest changes from ${repoUrl}...`);
 ...
 */
async function runScriptForProject(script: string, projectPath: string): Promise<{ status: string, projectPath: string, outputFilePath: string, stdoutText:string, errorFilePath: string }> {
  // Ensure the results directory exists
  const resultsDir = `./results`;
  ensureDirSync(resultsDir);

  // Define paths to store stdout and stderr outputs
  const outputFilePath = `${resultsDir}/${projectPath.replace(/\//g, "_")}_output.txt`;
  const errorFilePath = `${resultsDir}/${projectPath.replace(/\//g, "_")}_error.txt`;
  ...
  try {
    ...
    const runCommand = new Deno.Command("deno", {
      args: ["run", "--allow-read", "--allow-run", join(Deno.cwd(), script)],
      cwd: projectPath,
      stdout: "piped",
      stderr: "piped",
    });
    ...
    if (stderrText) console.error(`[${projectPath}] stderr:\n${stderrText}`);

    if (code === 0) {
      console.log(`Successfully ran ${script} for ${projectPath}`);
      return { status: "Success", projectPath, outputFilePath, stdoutText, errorFilePath };
    } else {
      console.log(`Script ${script} failed for ${projectPath} with exit code ${code}`);
      return { status: "Failed", projectPath, outputFilePath, stdoutText, errorFilePath };
    }
  } catch (error: any) {
    console.log(`Error running ${script} for ${projectPath}: ${error.message}`);
    await Deno.writeTextFile(errorFilePath, error.message);
    return { status: "Error", projectPath, outputFilePath, stdoutText: "", errorFilePath };
  }
}
 ...
 */
async function runScriptsForAllProjects(script: string, projects: string[]): Promise<void> {
  const table = new Table()
    .header(["Project Path", "Status", "Output", "Output File", "Error File"])
    .border(true);
...
async function generateScriptForQuestion(question: string, scriptName: string) {
  ...
  const prompt = Deno.readTextFileSync(pathToPrompt).replace("{{QUESTION}}", question);
  ...
  const generatedScript = extractTypeScriptCode(responseData.choices[0].message.content);
...
function saveProjects(projects: any) {
  Deno.writeTextFileSync("./projects.json", JSON.stringify(projects, null, 2));
}
...
async function runScript(scriptName: string) {
  const projects = loadProjects();
  await runScriptsForAllProjects(scriptName, projects.projects.map((project: any) => project.path));
}
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.error("Unknown command. Available commands: add, query, run, pr");
      Deno.exit(1);
  }
}
